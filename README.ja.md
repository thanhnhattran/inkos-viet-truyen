...
**v1.4.0 短編執筆と Studio Chat ワークフロー更新** — Studio Chat と CLI から独立した短編パッケージ、あらすじ、セールスポイント、表紙プロンプト、任意の表紙画像を生成できます。プロジェクトチャットはリロード後も保持され、生成物のプレビューと編集にも対応しました。Studio のモデル設定では、世界の主要モデル集約サービスとして [kkaiapi](https://en.kkaiapi.com/) と [OpenRouter](https://openrouter.ai/) を選択できます。
